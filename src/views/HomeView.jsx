...
function TripOverviewCard({ trip, isActive, onSelect }) {
  ...
  // Aggregate stats across trip days
  ...
  if (phase.phase === "during") {
    badge = { label: `Dag ${phase.dayOfTrip}/${phase.totalDays}`, bg: "#34D39922", color: "#34D399", dot: "#34D399" };
    ...
    >
      ...
      {status === "ready" && tripDays.length > 0 && (
        <>
          ...
          {/* Stats row */}
          <div style={{
            display: "flex", gap: 16, padding: "8px 14px 12px",
            borderTop: `1px solid ${colors.surfaceBorder}`,
            flexWrap: "wrap",
          }}>
            {avgWind != null && (
              <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                <span style={{ fontSize: 10, color: colors.textMuted }}>💨 Gem. wind</span>
                <span style={{ fontSize: 14, fontWeight: 800, color: colors.text }}>{avgWind} kn</span>
              </div>
            )}
            {avgTemp != null && (
              <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                <span style={{ fontSize: 10, color: colors.textMuted }}>🌡️ Gem. max temp</span>
                <span style={{ fontSize: 14, fontWeight: 800, color: colors.text }}>{avgTemp}°C</span>
              </div>
            )}
            <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 800, color: goodCount > 0 ? "#34D399" : colors.textMuted }}>
            ...
            {cancelCount > 0 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                <span style={{ fontSize: 10, color: colors.textMuted }}>
                  {isPastTrip ? "Geannuleerd" : "Kans op annulering"}
                </span>
                <span style={{ fontSize: 14, fontWeight: 800, color: "#EF4444" }}>{cancelCount}</span>
              </div>
            )}
          </div>
        </>
...
function SunWaterCard() {
  ...
  const months = ["jan","feb","mrt","apr","mei","jun","jul","aug","sep","okt","nov","dec"];
  const tripLabel = formatDateRange(activeTrip.startDate, activeTrip.endDate).slice(0,5);
    ...
    <Card>
      ...
      {loading ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 16px" }}>
          <StatRow emoji="🌄" label={isPast ? `Zonsopgang (${parseInt(activeTrip.startDate.slice(8))} ${months[parseInt(activeTrip.startDate.slice(5,7))-1]})` : "Zonsopgang"} value={sun?.rise ?? "–"} />
          ...
          <StatRow emoji="🏕️" label={`Gem. trip (${tripLabel})`} value={tripAvg != null ? `${tripAvg}°C` : "–"} color="#60A5FA" />
...
function TodoRow({ item, isChecked, onToggle }) {
  return (
    <div
      ...
      style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "7px 0", borderBottom: `1px solid ${colors.surfaceBorder}`, cursor: "pointer" }}
    >
      <div style={{
  ...
  return (
    <>
      ...
      <div className="page-content">
        {/* All trips overview — each card fetches its own weather */}
        <TripsOverview />

        {/* Active trip detail */}
        <div style={{ marginTop: 4 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: colors.textMuted, letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 10 }}>
            {activeTrip.flag ?? activeTrip.emoji} {activeTrip.name} — detail
          </div>
